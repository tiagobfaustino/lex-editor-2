...
2) Orient before reading:
- This repository enforces graphify-first navigation (`CLAUDE.md` plus the
  `PreToolUse` hooks in `.claude/settings.json`). When `graphify-out/graph.json`
  exists, start with `graphify query "<question>"`, `graphify explain
  "<concept>"` and `graphify path "<A>" "<B>"` to map components and edges,
  then read the specific files you must cite as evidence.
- If `graphify-out/` is absent, fall back to `Glob`/`Grep` plus targeted reads.
- Prefer the `Explore` subagent for broad fan-out sweeps; keep the evidence
  citations (file and line) in your own notes.
