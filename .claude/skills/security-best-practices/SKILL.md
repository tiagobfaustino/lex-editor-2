...
This repository enforces graphify-first orientation (see `CLAUDE.md` and the
`PreToolUse` hooks in `.claude/settings.json`). When `graphify-out/graph.json`
exists, run `graphify query "<question>"` to locate the relevant surfaces
before grepping or reading source files, then read those files directly.
...
If no relevant information is available in the skill's references directory, think a little bit about what you know about the language, the framework, and all well known security best practices for it. If you are unsure you can use `WebSearch`/`WebFetch` for current documentation on security best practices, or the `find-docs` skill for library-specific APIs.
