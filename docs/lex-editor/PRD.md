...
| RF-20 | Configuração de fontes de importação | O sistema deve permitir ao Administrador Técnico cadastrar e versionar novas fontes de importação compatíveis com adaptadores instalados, testar pelo caminho real e administrar ativação, pausa, arquivo e restauração sem executar regra fornecida pela UI. | Uma nova origem de teste compatível com o adaptador Planalto é cadastrada, testada e ativada pela UI; importador e worker usam a mesma revisão e o mesmo conjunto de artefatos. |
...
| `sources:list` / `sources:create-provider-revision` / `sources:create-binding-revision` | renderer → main | Lista DTOs paginados e solicita revisões imutáveis de provedor/vínculo; identidade e papel não vêm do renderer. |
| `sources:dry-run` / `sources:activate` / `sources:pause` / `sources:archive` / `sources:restore` | renderer → main | Solicita teste seguro e transições administrativas versionadas; o servidor revalida autorização, evidência e concorrência. |
| `sources:request-check` | renderer → main | Solicita verificação imediata idempotente e deduplicada para vínculo ativo. |
