...
/** Coleta exatamente o conjunto versionado no catálogo, sem descobrir URLs extras. */
export const fetchConfiguredPlanaltoSourceSet = async (
  ...
  return Promise.all(
    [...binding.artifacts]
      ...
      .map(async (artifact) => ({
        ...(await fetchOne(new URL(artifact.sourceUrl), ports)),
